...
struct fmt::formatter<nese::cpu::processor>
{
    ...
    static void append_instruction_bytes(auto& out, const nese::cpu::processor& cpu)
    {
        ...
        const nese::addr_t size = 1 + get_operand_size(nese::cpu::processor::get_opcode_addr_mode(cpu.get_byte_from_memory(cpu.get_registers().pc)));
    ...
    static void append_opcode(auto& out, const nese::cpu::processor& cpu)
    {
        ...
        const auto opcode_str = nese::cpu::processor::get_opcode_string(opcode);
    ...
    static void append_operand(auto& out, const nese::cpu::processor& cpu)
    {
        ...
        const nese::cpu::registers& registers = cpu.get_registers();
        const nese::cpu::addr_mode addr_mode = nese::cpu::processor::get_opcode_addr_mode(cpu.get_byte_from_memory(registers.pc));
        const nese::byte_t opcode = cpu.get_byte_from_memory(registers.pc);
        const nese::addr_t pc = registers.pc + 1;

        switch (addr_mode)
        {
        case nese::cpu::addr_mode::implied:
            break;

        case nese::cpu::addr_mode::accumulator:
            append_char(out, 'A');
            operand_str_size += 1;
            break;

        case nese::cpu::addr_mode::immediate:
            out = fmt::format_to(out, "#${:02X}", cpu.get_byte_from_memory(pc));
            operand_str_size += 4;
            break;

        case nese::cpu::addr_mode::relative:
            out = fmt::format_to(out, "${:04X}", static_cast<nese::word_t>(cpu.get_byte_from_memory(pc) + pc + 1));
            operand_str_size += 5;
            break;

        case nese::cpu::addr_mode::zero_page:
        case nese::cpu::addr_mode::zero_page_x:
        case nese::cpu::addr_mode::zero_page_y:
        {
            ...
            if (addr_mode == nese::cpu::addr_mode::zero_page_x)
            {
                addr += registers.x;
                out = fmt::format_to(out, ",X @ {:02X}", addr);
                operand_str_size += 7;
            }
            else if (addr_mode == nese::cpu::addr_mode::zero_page_y)
            {
                addr += registers.y;
        ...
        case nese::cpu::addr_mode::absolute:
        case nese::cpu::addr_mode::absolute_x:
        case nese::cpu::addr_mode::absolute_y:
        {
            ...
            if (nese::cpu::processor::get_opcode_string(opcode) != "jmp" && nese::cpu::processor::get_opcode_string(opcode) != "jsr")
            {
                if (addr_mode == nese::cpu::addr_mode::absolute_x)
                {
                    addr += registers.x;
                    out = fmt::format_to(out, ",X @ {:04X}", addr);
                    operand_str_size += 9;
                }
                else if (addr_mode == nese::cpu::addr_mode::absolute_y)
                {
                    addr += registers.y;
        ...
        case nese::cpu::addr_mode::indirect:
        {
            const nese::addr_t addr = cpu.get_word_from_memory(pc);

            if (nese::cpu::processor::get_opcode_string(opcode) != "jmp" || (addr & 0xff) != 0xff)
            {
        ...
        case nese::cpu::addr_mode::indexed_indirect:
        {
        ...
        case nese::cpu::addr_mode::indirect_indexed:
        {
            const nese::addr_t addr = cpu.get_byte_from_memory(pc);
